...
1. WHEN Clientが有効な Discord または Slack webhook URL を含む `PUT /settings/webhook` を送信したとき、THE API SHALL その URL をアプリ全体設定として保存し、保存結果を返す。
...
4. IF Clientが Discord または Slack webhook URL ではない URL を `PUT /settings/webhook` に送信したとき、THEN THE API SHALL HTTP ステータス 422 を返す。
...
12. THE API SHALL Discord と Slack の incoming webhook に対応する。
