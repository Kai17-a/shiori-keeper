...
- custom news site 登録は LLM 設定必須で、URL-only の LLM selector 生成、API による HTML 取得、実抽出テストの成功後だけ `news_sites` へ保存する
- selector 生成には外部 URL を閲覧できる model または互換 server が必要で、通常の chat-only model は対応しない
