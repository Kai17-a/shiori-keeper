    ...
    <div class="page">
        ...
        <section class="panel">
            <form class="form" @submit.prevent="createFolder">
                <input v-model="folderName" placeholder="New folder" required />
        ...
        <section class="panel">
            <div class="list">
                <article v-for="folder in folders" :key="folder.id" class="item">
                    <div class="row">
                        <div class="row-main">
                        ...
                        <div class="actions">
                            <button type="button" class="edit" @click="openEdit(folder)">Edit</button>
                            <button
                                type="button"
                                class="trash"
        ...
        <div v-if="editOpen" class="modal-backdrop" @click.self="closeEdit">
            <div class="modal">
                ...
                <form class="form modal-form" @submit.prevent="saveEdit">
                    <input v-model="editForm.name" placeholder="Folder name" required />
                    <button>Save</button>
                </form>
            </div>
        </div>
        ...
        <div v-if="confirmOpen" class="modal-backdrop" @click.self="closeConfirm">
            <div class="modal">
                ...
                <p class="confirm-text">
                    Delete <strong>{{ pendingFolder?.name }}</strong> permanently?
                </p>
...
const createFolder = async () => {
    try {
        ...
        if (!name) {
            message.value = "Folder name is required.";
            return;
        }
...
const confirmDelete = async () => {
    ...
    try {
        await request(`/folders/${pendingFolder.value.id}`, { method: "DELETE" });
        closeConfirm();
        await refresh();
    } catch (err) {
        message.value = err.message;
