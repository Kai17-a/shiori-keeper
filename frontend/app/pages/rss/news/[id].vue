<template>
  <UDashboardPanel id="news-site-detail">
    <template #header>
      <PageHeaderActions title="Custom news site" />
    </template>
    ...
    <template #body>
      <div class="space-y-6">
        <UPageCard>
          <UAlert
            v-if="state === 'error'"
            title="Failed to load custom news site"
          ...
          <UAlert
            ...
            title="Custom news site not found"
        ...
        <UPageCard
          ...
          description="Articles are stored after at least one webhook delivery succeeds"
          :ui="{ body: 'space-y-4' }"
        >
          ...
          >
            ...
            <p v-else>No scraped articles recorded yet.</p>
          </div>
...
const loadSite = async () => {
  try {
    ...
    const message = err instanceof Error ? err.message : "Failed to load custom news site.";
...
const loadArticles = async (showToast = false) => {
  ...
  try {
    ...
    if (showToast) {
      toast.show({
        title: "Scraped articles refreshed.",
        color: "success",
        icon: "i-lucide-check",
      });
    }
    ...
    toast.show({
      title: "Failed to load scraped articles.",
      description: err instanceof Error ? err.message : undefined,
      color: "error",
      icon: "i-lucide-circle-alert",
    });
...
const saveSite = async () => {
  ...
  try {
    ...
    toast.show({
      title: "Custom news site updated.",
      color: "success",
      icon: "i-lucide-check",
    });
  } catch (err) {
